...
//                .flatMap(user -> messageRepository.findAll(user)
//                        .flatMap(mention ->
//                                Flux.zip(Flux.just(mention.getMessage().getFrom().getName()), Flux.just(mention.getMessage().getId()), Flux.just(mention.getUser().getId())))
    ...
    public Mono<ServerResponse> createMention(ServerRequest request) {
        ...
        return userRepository.findById(userId)
                .flatMap(user -> messageRepository.findAllById(List.of("5ca9aa5d9fa1a570ace64a3f", "5ca9a9ea9fa1a570ace64a3c", "5c8e0bc05eb70d9bcfa8be9f"))
                        .flatMap(message -> mentionRepository.save(Mention.builder().user(user).build()))
                        .collectList())
        ...
        return request.bodyToMono(ReadMentionPayload.class)
                .flatMap(mentionPayload -> {
                            ...
                            String userId = Jwts.parser().setSigningKey(MyChatServerApplication.SECRET_KEY)
                                    ...
                                    .getBody().getId();
                            return Mono.zip(userRepository.findById(userId), Mono.just(mentionPayload.getMessageId()));
                        }
                ).flatMap(t ->
                        messageRepository.findById(t.getT2())
                                .map(message -> Tuples.of(message, message.getMentions()))
                                .map(messageTuple2 ->
                                {
                                    messageTuple2.getT2().stream().filter(mention -> mention.getUser().equals(t.getT1()))
                                            .map(mention -> {
                                                mention.read();
                                                return mention;
                                            })
                                            .collect(Collectors.toList());
                                    return messageTuple2.getT1();
                                })
                )
